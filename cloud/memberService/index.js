...
async function addSign(event) {
  ...
  try {
    let memberInfos = await db.collection('dg_member').where({
    ...
    if (memberInfos.data.length === 0) {
      let task1 = db.collection('dg_member').add({
        data: {
          openId: event.info.openId,
    ...
    let task3 = db.collection('dg_sign_detail').add({
      data: {
        openId: event.info.openId,
        year: date1[0],
        month: date1[1],
    ...
    if (event.info.accept == 'accept') {
      let task4 = await db.collection("dg_subcribute").add({
        data: {
    ...
    //积分明细
    let task5 = db.collection('dg_point_detail').add({
      data: {
        openId: event.info.openId,
...
async function getSignedDetail(event) {
  ...
  console.info(event)
  let res = await db.collection('dg_sign_detail')
    .where({
...
async function applyVip(event) {
  ...
  try {
    ...
    if (memberInfos.data.length === 0) {
      await db.collection('dg_member').add({
        data: {
          openId: wxContext.OPENID,
          totalSignedCount: 0,//累计签到数
          continueSignedCount: 0,//持续签到
      ...
      let memberInfo = memberInfos.data[0]
      await db.collection('dg_member').doc(memberInfo._id).update({
        data: {
          applyStatus: 1,
...
async function addShareDetail(event) {
  ...
  let shareInfos = await db.collection('dg_share_detail').where({
    openId: wxContext.OPENID,
  ...
  if (shareInfos.data.length > 0) {
    ...
    try {
      ...
      let task3 = db.collection('dg_share_detail').add({
        data: {
          shareOpenId: event.info.shareOpenId,
          openId: wxContext.OPENID,
          avatarUrl: event.info.avatarUrl,//头像
