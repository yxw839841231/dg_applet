...
async function sendTemplateMessage(event) {
  ...
  var openIdformIds = await db.collection('dg_formids').where({
    openId: openId
  }).limit(1).get()
  ...
  const removeResult = await db.collection('dg_formids').doc(openIdformIds.data[0]['_id']).remove()
...
async function querySubscribeCount(event) {
  var data = {}
  var formIdsResult = await db.collection('dg_subcribute').where({
...
async function removeExpireFormId(event) {
  try {
    return await db.collection('dg_formids').where({
...
async function addSubscribeCount(event) {
  try {
    ...
    for (var i = 0; i < event.templateIds.length; i++) {
      ...
      let res = await db.collection('dg_subcribute').add({
        data: data
      })
      console.info(res)
    }
