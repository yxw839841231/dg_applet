...
async function addPostQrCode(event) {
  ...
  if (result.errCode === 0) {
    ...
    await db.collection("dg_posts").doc(event.postId).update({
      data: {
...
async function addPostComment(event) {
  ...
  let task = db.collection('dg_posts').doc(event.commentContent.postId).update({
    data: {
  ...
  await db.collection("dg_comments").add({
  ...
  if (event.accept == 'accept') {
    await db.collection("dg_subcribute").add({
      data: {
...
async function addPostChildComment(event) {
  ...
  let task = db.collection('dg_posts').doc(event.postId).update({
    data: {
      totalComments: _.inc(1)
    }
  });
  ...
  await db.collection('dg_comments').doc(event.id).update({
    data: {
...
async function addPostCollection(event) {
  ...
  console.info(event)
  let postRelated = await db.collection('dg_posts_related').where({
  ...
  if (postRelated.data.length === 0) {
    ...
    let result = await db.collection('dg_posts_related').add({
      data: {
        openId: event.openId == undefined ? event.userInfo.openId : event.openId,
        postId: event.postId,
...
async function addPostZan(event) {
  ...
  if (postRelated.data.length === 0) {
    await db.collection('dg_posts_related').add({
      data: {
        openId: event.openId == undefined ? event.userInfo.openId : event.openId,
        postId: event.postId,
        postTitle: event.postTitle,
...
async function deletePostCollectionOrZan(event) {
  db.collection('dg_posts_related').where({
    openId: event.openId == undefined ? event.userInfo.openId : event.openId,
    postId: event.postId,
    type: event.type
  }).remove()
...
async function getPostsDetail(event) {
  ...
  console.info(event)
  let post = await db.collection("dg_posts").doc(event.id).get()
  if (post.code) {
    return "";
  }
  if (!post.data) {
  ...
  const tasks = []
  //获取文章时直接浏览量+1
  let task = db.collection('dg_posts').doc(event.id).update({
    data: {
      totalVisits: _.inc(1)
    }
  })
  ...
  /*let task2 = db.collection('dg_member').doc(memberInfo._id).update({
    data: {
  ...
  //积分明细
  let task3 = db.collection('dg_point_detail').add({
    data: {
