...
Page({
  ...
  onReachBottom: async function() {
    ...
    try {
      ...
      var urlContent = config.host + '/api/content/posts/' + that.data.post.id;
      var urlComments = urlContent + '/comments/list_view';
      await request.requestGetApi(urlComments, config.token, {
          page: page - 1
        }, this,
        function(res, selfObj) {
          ...
          if (list.length != 0) {
            for (let i = 0; i < list.length; ++i) {
              ...
              if (list[i].parent){
                list[i].parent.createTime = util.customFormatTime(list[i].parent.createTime, 'Y-M-D  h:m:s');
              }
              list[i].falg = true;
              if (list[i].isAdmin) {
                list[i].email = '';
                list[i].authorUrl = 'https://image.dgjava.com/bitbug_favicon_1585759457616.ico';
              }
            }
            list[list.length - 1].falg = false;
            let nomore = res.data.page+1==res.data.pages;
            that.setData({
              commentPage: page + 1,
  ...
  getDetail: async function(blogId) {
    ...
    await request.requestGetApi(
      config.host + "/api/content/posts/" + blogId,
      config.token, {},
      this,
      function(res, selfObj) {
        console.log(res)
        ...
        let postDetail = res;
        postDetail.data.createTime = util.customFormatTime(postDetail.data.createTime, 'Y.M.D');
        that.setData({
  ...
  postCollection: async function() {
    ...
    try {
      ...
      if (collection.status === true) {
        let result = await api.deletePostCollectionOrZan(that.data.post.id, config.postRelatedType.COLLECTION, app.globalData.openid)
        console.info(result)
        that.setData({
          collection: {
            status: false,
        ...
        await request.requestPostApi(
          config.host + "/api/content/posts/" + that.data.post.id + "/unenshrine",
          config.token, {},
          this,
          function(res, selfObj) {
            wx.showToast({
        ...
        let data = {
          ...
          postUrl: that.data.post.thumbnail,
          postDigest: that.data.post.summary,
        ...
        await request.requestPostApi(
          config.host + "/api/content/posts/" + that.data.post.id + "/enshrine",
          config.token, {},
          this,
          function() {
            wx.showToast({
  ...
  postZan: async function() {
    ...
    try {
      ...
      if (zan.status === true) {
        await api.deletePostCollectionOrZan(that.data.post.id, config.postRelatedType.ZAN, app.globalData.openid)
        that.setData({
          zan: {
            status: false,
            text: "点赞",
        ...
        request.requestPostApi(
          config.host + "/api/content/posts/" + that.data.post.id + "/unlikes",
          config.token, {}, this, null, null);
        ...
        request.requestPostApi(
          config.host + "/api/content/posts/" + that.data.post.id + "/likes",
          config.token, {}, this, null, null);
  ...
  formSubmit: async function(e) {
    try {
      ...
      var urlPostList = config.host + '/api/content/posts/comments';
      var params = {
        author: that.data.userInfo.nickName,
        authorUrl: that.data.userInfo.avatarUrl,
        content: that.data.commentContent,
        email: app.globalData.openid,
      ...
      await request.requestPostApi(
        ...
        function(res, selfObj) {
          ...
          var urlContent = config.host + '/api/content/posts/' + that.data.post.id;
          var urlComments = urlContent + '/comments/list_view';
           request.requestGetApi(urlComments, config.token, {
            page: 0
          }, this,
            function (res, selfObj) {
              var list = res.data.content;
  ...
  onCreatePoster: async function() {
    ...
    let imageUrl = that.data.post.thumbnail
    let qrCodeUrl = imageUrl;
    var images = [{
        width: 62,
        height: 62,
        x: 32,
      ...
      },
      {
        ...
        x: 70,
        y: 1000,
        url: "https://image.dgjava.com/xcx50_1586744365757.jpg", //二维码的图
      }
    ];
